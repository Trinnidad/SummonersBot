...
function SWBot(){
	...
	this.giveHellos = function(me, amount) {
		time = new Date().getTime() / 1000 ;
		...
		var sql = this.connection.query('SELECT * FROM hellos order by hellos DESC',
			function(err, result){
				if (err) {
					...
					if (result.length){
						var messages = [];
						messages.push(function(cb){me.bot.sendMessage(channel, "      HELLOS LEADERBOARD"); cb();});
...
SWBot.prototype.listen = function () {
	var me = this;
	setInterval(this.giveHellos, 60000, me,1);
	this.bot.on("message", function (msg) {
		if (msg.author.name != this.user.name){
			...
			switch(command[0]){
				...
				case '!giveall':
					if (settings.discordMods.indexOf(msg.author.name) != -1) {
						me.giveHellos(me, parseInt(command[1],10));
						me.bot.sendMessage(msg.channel, "Gave " + command[1] + " hellos to everyone. Like hello?");
					}
					break;
				case '!leaderboard':
